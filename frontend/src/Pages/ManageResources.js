...
const ResourceItem = ({ resource, onApprove, onReject }) => {
  ...
  const handleApprove = async () => { setLoading(true); await onApprove(resource._id, category, reviewNotes); setLoading(false); };
  const handleReject  = async () => { setLoading(true); await onReject(resource._id, reviewNotes); setLoading(false); };
      ...
      <div className="mr-root">
        ...
        <div className="mr-body">
          ...
          <div className="mr-list">
            ...
            {loading ? (
              <div className="mr-loading"><div className="mr-spinner" /><p>Loading pending resources…</p></div>
            ) : resources.length === 0 ? (
              <div className="mr-empty">
                <div className="mr-empty-ico"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22,4 12,14.01 9,11.01"/></svg></div>
                <h4>All caught up!</h4>
              ...
              <>
                {resources.map(resource => (
                  <ResourceItem key={resource._id} resource={resource} onApprove={handleApprove} onReject={handleReject} />
                ))}
                {totalPages > 1 && (
